...
Connections2Triples.prototype._transform = function (data, encoding, done) {
  switch (data.type) {
    ...
    case 'route':
      ...
      break;
    case 'stop':
      this._transformStop(data, encoding, done);
      break;
    default:
...
Connections2Triples.prototype._transformStop = function (stop, encoding, done) {
  var stopId = this._baseUris.stops + encodeURIComponent(stop.stop_id);
  var stationId = this._baseUris.stations + encodeURIComponent(stop.parent_station || stop.stop_id);
  var stationLink = stop.station;
  this.push({
    subject : stopId,
    predicate : 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type',
  ...
  this.push({
    ...
    object : "\"" + stop.stop_lat + "\"^^http://www.w3.org/2001/XMLSchema#decimal"
  });
  this.push({
    subject : stopId,
    predicate : 'http://www.w3.org/2003/01/geo/wgs84_pos#long',
    object : "\"" + stop.stop_lon + "\"^^http://www.w3.org/2001/XMLSchema#decimal"
  });
  this.push({
    subject : stopId,
    predicate : 'http://vocab.gtfs.org/terms#platformCode',
    object : "\"" + (stop.platform_code || 0) + "\""
  });
  this.push({
    subject : stopId,
  ...
  if (stationLink) {
    ...
    this.push({
      subject: stationId,
      predicate: 'http://www.w3.org/2003/01/geo/wgs84_pos#lat',
      object: "\"" + stationLink.station_lat + "\"^^http://www.w3.org/2001/XMLSchema#decimal"
    });
    this.push({
      subject: stationId,
      predicate: 'http://www.w3.org/2003/01/geo/wgs84_pos#long',
      object: "\"" + stationLink.station_lon + "\"^^http://www.w3.org/2001/XMLSchema#decimal"
    });
