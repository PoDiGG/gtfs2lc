var csv = require('fast-csv'),
    ...
    StopsBuilder = require('./StopsBuilder.js'),
    ...
    if (count === (2 + !!self._enrich + !!delays)) {
      ...
      connectionsStream.on('end', function () {
        if (self._enrich) {
          ...
          routes.pipe(new RoutesBuilder(self._options)).pipe(combinedStream, { end: false });
          routes.on('end', function() {
            process.stderr.write("\n");
            var stops2 = fs.createReadStream(path + '/stops.txt', {encoding:'utf8', objectMode: true}).pipe(csv({objectMode:true,headers: true}));
            stops2.on('error', function(e){ console.error('Error reading stops.txt: ' + e); });
            stops2.pipe(new StopsBuilder(stopsdb, self._options)).pipe(combinedStream);
          });
        } else {
